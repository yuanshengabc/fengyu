...
def run_start(m_mode):
    ...
    if status['status'] is 'started':
        sys.stdout.write("mysql-%s is running.\n" % m_mode)
        return True
...
def run_stop(m_mode):
    ...
    if status['status'] is 'stoped':
        sys.stdout.write("mysql-%s is not running.\n" % m_mode)
        return True
...
def main():
    ...
    if os.path.isfile(conf_fname):
        ...
        if conf.mysql_home:
            mysql_home = conf.mysql_home
        if conf.service_name:
            service_name= conf.service_name

    if not mysql_home:
        mysql_home = sys.path[0] + "/../"
